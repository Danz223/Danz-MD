    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          this.reply(m.chat, `[❗] Limit kamu abis dek, beli melalui *${usedPrefix}buy limit*`, m)
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          try {
            ...
            conn.sendButtonImg(id, await (await fetch(action === 'add' ? wel : lea)).buffer(), 'RellBots MD', text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'RellBots-MD', fkontak, {
              contextInfo: {
                externalAdReply: {
...
export async function deleteUpdate(message) {
  try {
    ...
    await conn.send2ButtonDoc(msg.chat, `Terdeteksi @${participant.split`@`[0]} telah menghapus pesan*`, '\nMematikan fitur ini, Klik Button dibawah', 'Disable Antidelete', '.disable antidelete', 'Owner', '.Owner', msg, {
      contextInfo: {
        externalAdReply: {
          title: 'Hᴀʟᴏ BᴏsQ><',
          body: wm,
...
global.dfail = (type, m, conn) => {
  ...
  if (rown) return conn.sendButtonDocAccess(m.chat, `${ucapan()} ` + tag, rown + '\n\n\n\n', 'Owner', '.owner', m, {
    contextInfo: {
      externalAdReply: {
        showAdAttribution: true,
        mediaUrl: 'https://discord.gg/NzNmQhBMkq',
        mediaType: 1,
        renderLargerThumbnail: false,
        description: sgc,
