...
let handler = async (m, { conn, usedPrefix: _p, __dirname, args }) => {
  ...
  try {
    ...
    const sections = [
      {
        ...
        title: `${htki} ◈  •══▶ Store ◀══•  ◈ ${htka}`,
        rows: [
          { title: `🛒 ${pmenus} Store`, rowId: ".? store", description: "Belum Menjual Apa-Apa" },
        ]
      }, {
        title: `${htki}   ◈  ◈  •══▶ Menu ◀══•  ◈   ◈ ${htka}`,
        rows: [
          ...
          { title: `🔞 ${pmenus} Nsfw`, rowId: ".? nsfw", description: "Warning 18+ Only" },
    ...
    let con = `╭─────═[ INFO USER ]═─────⋆
      ...
      │⇌  Sᴛᴀᴛᴜs : ${m.sender.split`@`[0] == nomorown ? 'Developer' : (usrs.premiumTime >= 1 ? 'Premium' : 'Free')} 
      │⇌  Rᴏʟᴇ : ${usrs.role} 
      │⇌  Lɪᴍɪᴛ : ${usrs.limit}
      ╰────────────────···`
    let tett = `╰─────═[ Danz-MD ]═─────⋆`
    let fot = `Nᴏᴛᴇ❗ : Bot Masih Tahap Pengembangan Jadi Mohom Dimaklumi jika ada beberapa bug`
    const listMessage = {
      text: tett,
      footer: fot,
    ...
    let after = conn.menu.after || (conn.user.jid == global.conn.user.jid ? '' : `Powered by https://wa.me/62881026950162`) + defaultMenu.after
    let _text = [
      before,
    ...
    let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `DanzBotzMD`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` } } }
    ...
    let almenu = `▢ ▱▱▱ ▣► DanzBotz-Menu ◄▣ ▱▱▱ ▢`
    let nomorwa = '0'
    let nomorowm1 = '0881026950162'
    ...
    conn.sendButtonDoc(m.chat, almenu, text + `${ᴛᴇs}`, 'Oᴡɴᴇʀ', '.owner', m, { contextInfo: { forwardingScore: fsizedoc, externalAdReply: { body: 'Danz', containsAutoReply: true, mediaType: 1, mediaUrl: hwaifu.getRandom(), renderLargerThumbnail: false, showAdAttribution: true, sourceId: 'Danzbotz', sourceType: 'PDF', previewType: 'PDF', sourceUrl: sgc, thumbnail: fs.readFileSync('./thumbnail.jpg'), thumbnailUrl: sgc, title: 'Danz-Menu' } } })
